...
import page.AtencionCliente.EmitirActivarBloquear;
import util.ALM;
import util.Evidencia;
import util.FunctionGeneric;
import util.LeerExcel;

public class TC003_Activar_Clave {
	...
	private EmitirActivarBloquear emiAcBl;
	...
	public void Test() {
		...
		try {
			...
			if (!FunctionGeneric.stateStep("Menú tarjetas", estado, ITestCaseRun, wrapper)) {
				flagState = false;
				afterClass();
			}

			estado = emiAcBl.activarTarjeta(driver);
			if (!FunctionGeneric.stateStep("Activar Tarjeta", estado, ITestCaseRun, wrapper)) {
				flagState = false;
				afterClass();
			}
